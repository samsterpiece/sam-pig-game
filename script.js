...
let scores, currentScore, activePlayer, playing, diceSideNumber;
...
const setLifetimeScore = function (player, currentScore, currentLifetimeScore) {
  localStorage.setItem(
    `lifetime-score-${player}`,
    currentLifetimeScore + currentScore
  );
  document.querySelector(`.life--${player}`).textContent =
    "Lifetime Score: " + (currentLifetimeScore + currentScore);
};
  ...
  if (playing) {
    ...
    setTimeout(
      function () {
        ...
        //Generate Dice Variables 1 to 6, can expand to more than 6-sided if desired(change 6 to 7, or 7 to 8, 9 for )
        const dieOneValue = Math.floor(Math.random() * diceSideNumber) + 1;
        const dieTwoValue = Math.floor(Math.random() * diceSideNumber) + 1;
        ...
        if (
          ...
          document.getElementById(`score--${activePlayer}`).textContent =
            "Total Score: " + scores[activePlayer]; //Adds to page of update
          switchPlayer(); //changes player
        } else if (dieOneValue === dieTwoValue) {
          //Case 3: Matching dies
          snakeEye.classList.add("hidden");
          ...
          document.getElementById(`current--${activePlayer}`).textContent =
            "Turn Score: " + currentScore; //doubles sum reflected in current score
        } else {
          //Default case
          snakeEye.classList.add("hidden");
          btnHold.disabled = false;
